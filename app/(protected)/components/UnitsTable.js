...
{!loading && (
    ...
    {!loading && (
      <Box sx={{ mb: 4, mt: 3, width: '100%' }}>
        <Typography variant="subtitle1" mb={2}>Resumen por cliente</Typography>
  ...
  <Box sx={{ mb: 4 }}>
    <Paper sx={{ p: 3, bgcolor: '#fafafa' }}>
      ...
      <Typography variant="caption" color="text.secondary">
        ...
        {selectedUser && ` filtradas para el cliente  `}<strong style={{ textTransform: 'capitalize' }}>{selectedUser}</strong>
        {selectedType && `, `}<strong>{unitTypes[selectedType]}</strong>
        {selectedStatus && `, `}<strong>{ESTADOS[selectedStatus]?.label}</strong>
        {selectedTypology && `, `}<strong>{selectedTypology}</strong>
